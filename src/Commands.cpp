...
/*   Updated: 2025/01/20 16:50:49 by thlefebv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::parseCommand(const std::string& message, int client_fd)
{
    ...
    while (!cleanMessage.empty() && 
           (cleanMessage[cleanMessage.size() - 1] == '\r' || cleanMessage[cleanMessage.size() - 1] == '\n'))
    {
        cleanMessage.erase(cleanMessage.size() - 1); // Supprime le dernier caractere
    }
	...
	else if (command == "JOIN")
    {
        ...
        {
            std::ostringstream errorMsg;
            errorMsg << ":irc.server 461 " << client_fd << " JOIN :Not enough parameters\n";
		...
		{
			std::ostringstream errorMsg;
            errorMsg << ":irc.server 476 " << client_fd << " " << channelName << " :Invalid channel name\n";
            send(client_fd, errorMsg.str().c_str(), errorMsg.str().size(), 0);
            return;
		}
        CommandParams params;
...
else if (command == "INVITE")
{
    ...
    {
        std::ostringstream errorMsg;
        errorMsg << ":irc.server 461 " << client_fd << " INVITE :Not enough parameters\n";
        send(client_fd, errorMsg.str().c_str(), errorMsg.str().size(), 0);
    }
    else
    {
        ...
        params.channelName = channelName;
        params.nickname = nickname;
        params.Arg = std::vector<std::string>(); // Vecteur vide pour Arg
        params.additionalParams = std::vector<std::string>(); // Vecteur vide pour additionalParams
...
void Server::handleConnectionCommands(const std::string& command, std::istringstream& iss, int client_fd)
{
	...
	else if (command == "USER")
	{
		...
		std::getline(iss, realname); // Récupère le nom réel

		// Supprime les espaces blancs en début de chaîne
		...
		// Attribuez les informations utilisateur
		for (size_t i = 0; i < clients.size(); ++i)
		{
			if (clients[i]->getFd() == client_fd)
			{
				clients[i]->setName(realname); // Définit le nom réel
