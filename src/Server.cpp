...
/*   Updated: 2025/01/13 15:52:35 by mehdi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
 ...
 * @brief Create a server socket
	...
	It also sets the `SO_REUSEADDR` option,
		which allows the server to reuse the address without waiting for the operating system to release the port.
 ...
 * @socket:
	...
	- 0 = Automatically selects the appropriate protocol (TCP for SOCK_STREAM,
			UDP for SOCK_DGRAM)
...
Server::Server(int port, const std::string &password) : is_running(true)
// Init flag to make the serv run
{
	this->port = port;
...
void Server::handle_signal(int signal)
{
	...
	for (size_t i = 0; i < client_fds.size(); ++i)
		send(client_fds[i], shutdown_message.c_str(), shutdown_message.size(),
			0);
	close_all_clients();
	close(server_fd);
	exit(0);
...
int Server::CreateSocket()
{
	...
	tmp_opt = 1;
	if (setsockopt(this->server_fd, SOL_SOCKET, SO_REUSEADDR, &tmp_opt,
			sizeof(tmp_opt)) < 0)
	{
		...
		for (int fd = 0; fd <= max_fd; ++fd)
		{
			if (FD_ISSET(fd, &temp_fds))
			{
				if (fd == server_fd)
				{
					...
					std::string nameRequest = "Veuillez fournir votre nom : ";
					send(client_fd, nameRequest.c_str(), nameRequest.size(), 0);
					bytes_read = recv(client_fd, buffer, sizeof(buffer) - 1, 0);
					buffer[bytes_read] = '\0';
					std::string clientName(buffer);
					std::string nicknameRequest = "Veuillez fournir votre surnom : ";
					send(client_fd, nicknameRequest.c_str(), nicknameRequest.size(), 0);
					bytes_read = recv(client_fd, buffer, sizeof(buffer) - 1, 0);
					buffer[bytes_read] = '\0';
					...
					if (client_fd > max_fd)
						max_fd = client_fd;
					std::cout << COLOR_GREEN << "Nouvelle connexion acceptée (FD: " << client_fd << ", Name: " << clientName << ", Nickname: " << clientNickname << ")." << COLOR_RESET << std::endl;
				...
				else
				{
					...
					else if (bytes_read == 0) // Le client a fermé la connexion
					{
						...
						client_fds.erase(std::remove(client_fds.begin(),
								client_fds.end(), fd), client_fds.end());
					}
	...
	const std::string &nickname)
{
	...
	newClient = new Client(client_fd, name, nickname);
	clients.push_back(newClient);
	std::cout << "Client added : FD = " << client_fd << ", Name = " << name << ", Nickname = " << nickname << std::endl;
}
